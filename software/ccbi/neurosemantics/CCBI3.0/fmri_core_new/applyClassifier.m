% Use a learnt classifier (from trainClassifier) to label new examples
%
% Will also output a matrix of scores of the labelling over all the
% classes present in the training set. The columns of that matrix
...
% - classifier to use (eventually, this ought to be stored with the model)
%
% Out:
% - a SCORE array where each cell contains a matrix with
%   dimensions #examples * #classes. Each row contains
%   [score(1st class|data) ... score(kth class|data)]
%   For Naive Bayes, for instance, the score is ln(p(class|example)).
%   The order of the columns is the order of the class numeric
%   labels.
%
% - otherInfo is a cell array containing different pieces of
%   information depending on the methods used.
%
% Dependencies:
% - serious dependence on the "models" format produced by trainClassifier.
%
% History:
% - 10 Mar 05 - fpereira - adapted from previous code
% - 29 Jan 06 - vlm - pasted in fpereira new svml wrapper
% Ex:
% - [classifier] = trainClassifier(examples,labels);

function [scores,otherInfo] = applyClassifier( varargin )
...
switch classifierType
 ...
 case {'logisticRegression', 'SMLR','logisticRegressionMulti'}
 ...
 case {'svmlight'}
  ...
  % models{nClasses+1} is a cell array, in this case

  np  = length(models{nClasses+1}); % if 1, a two class problem
  tmp = zeros(nExamples,nClasses);
  
  % apply each one vs rest classifier and store results

  dummyLabels = ones(size(examples,1),1);  
  for l = 1:np
    fprintf('\t%d\n',sortedLabelValues(l));

    % apply it -> scores over that condition pair
    [discard, tmp(:,l) ] = svmclassify(examples,dummyLabels,models{nClasses+1}{l});
  end
    
  if np == 1
    % 2 class problem, now both 2 and multiclass have the same form
    % without needing to train an extra SVM
    tmp(:,2) = -1 * tmp(:,1);
    np = np + 1;
  end
