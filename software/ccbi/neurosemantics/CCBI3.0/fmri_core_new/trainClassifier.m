...
% - 01 Jan 06 - vlm - merged with fpereira latest svml wrapper
...
switch classifierType
 ...
 case {'logisticRegression'}
  ...
  case {'logisticRegressionMulti'}
  fprintf('trainClassifier: using %s with parameters\n',classifierType);
  disp(classifierParameters);  
  [models] = classifierLogisticRegressionMulti(examples,labels,'regularization',classifierParameters); 
 case {'SMLR'}
  fprintf('trainClassifier: using %s with parameters\n',classifierType);disp(classifierParameters);  
...
%  fprintf('trainClassifier: using %s with parameters\n',classifierType);disp(classifierParameters);  
 ...
 case {'svmlight'}
  ...
  % Uses the SVM-MEX interface for Thorsten Joachim's SVMlight.

  % If the current problem is two class, use classifierSVMlight as usual.
  % If multiclass, then train one classifierSVMlight for each one-vs-others
  % class problem. In both cases, result is packed into a cell array.

  if nClasses == 2    
    models{nClasses+1} = {classifierSVMlight( examples,labels,classifierParameters)};
  else
    models{nClasses+1} = cell(nClasses,1);
    ...
    for l = 1:nClasses
      ...
      mask = (labels == label);
      indicesIn  = find(mask);
      indicesOut = find(~mask);
      
      hereLabels = zeros(numTrain,1);
      hereLabels(indicesIn)  = 2;
      hereLabels(indicesOut) = 3;
      
      models{nClasses+1}{l} = classifierSVMlight( examples,hereLabels,classifierParameters );;
    end
  end
